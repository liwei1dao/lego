...
func newClient(pool *TcpConnPool, options *Options, conn net.Conn) (client *Client, err error) {
	client = &Client{
		...
		conn:    conn,
		hbeat:   0,
		state:   int32(connpool.ClientShakeHands),
	}
	go client.serveConn()
	return
}
...
func (this *Client) Start() {
	atomic.StoreInt32(&this.state, int32(connpool.ClientRuning))
	return
}
...
func (this *Client) Close() (err error) {
	if atomic.CompareAndSwapInt32(&this.state, int32(connpool.ClientRuning), int32(connpool.ClientClose)) {
		this.conn.Close()
	}
	return
}
...
locp:
	for {
		...
		go this.pool.host.Handle(this, req)
	}
}
