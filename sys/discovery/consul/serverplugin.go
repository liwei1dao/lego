...
func WithConsulOptions(options *discovery.Config) ConsulOpt {
	return func(o *ConsulRegisterPlugin) {
		o.Options = options
	}
}
...
func (p *ConsulRegisterPlugin) Start() error {
	...
	if p.kv == nil {
		kv, err := discovery.NewStore(discovery.CONSUL, p.ConsulServers, p.Options)
		if err != nil {
...
func (p *ConsulRegisterPlugin) Register(name string, rcvr interface{}, metadata string) (err error) {
	...
	if p.kv == nil {
		consul.Register()
		kv, err := discovery.NewStore(discovery.CONSUL, p.ConsulServers, nil)
		if err != nil {
			log.Errorf("cannot create consul registry: %v", err)
			return err
		}
