...
func NewRedisDiscovery(basePath string, servicePath string, options *Options) (*RedisDiscovery, error) {
...
func NewRedisDiscoveryTemplate(basePath string, redisAddr []string, options *Options) (*RedisDiscovery, error) {
	if len(basePath) > 1 && strings.HasSuffix(basePath, "/") {
		basePath = basePath[:len(basePath)-1]
	}
