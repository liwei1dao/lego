...
func newWeightedICMPSelector(servers map[string]string) ISelector {
...
func (s weightedICMPSelector) Select(ctx context.Context, service string, args interface{}) string {
	ss := s.servers
	if len(ss) == 0 {
		return ""
	}
