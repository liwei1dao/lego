...
func NewXClient(service string, selectMode SelectMode, discovery discovery.IServiceDiscovery, cpools rpc.IConnPool) IClient {
	client := &Client{
		discovery: discovery,
		service:   service,
	}
...
func (this *Client) Call(ctx context.Context, req interface{}, reply interface{}) (err error) { //同步调用 等待结果
	seq := new(uint64)
	ctx = WithValue(ctx, CallSeqKey, seq)
	stime := time.Now()
...
func (this *Client) call(ctx context.Context, args interface{}, reply interface{}) (call *MessageCall, err error) {
	...
	this.mutex.Unlock()
	if cseq, ok := ctx.Value(CallSeqKey).(*uint64); ok {
		*cseq = seq
	}
	var client rpc.IConnClient
...
func (this *Client) getclient(ctx context.Context) (client rpc.IConnClient, err error) {
	var (
		values string
...
func (this *Client) watch(ch chan []*rpc.KV) {
	for pairs := range ch {
		sort.Slice(pairs, func(i, j int) bool {
			return strings.Compare(pairs[i].Key, pairs[j].Key) <= 0
		})
...
func (this *Client) send(client rpc.IConnClient, call *MessageCall, seq uint64) (err error) {
	var (
		data    []byte
		allData *[]byte
	)
	...
	if len(data) > 1024 && this.options.CompressType != rpc.CompressNone {
...
func (this *Client) handleresponse(ctx context.Context, res rpc.IMessage) {
	var call *MessageCall
	seq := res.Seq()
	isServerMessage := (res.MessageType() == rpc.Request && !res.IsHeartbeat() && res.IsOneway())
	if !isServerMessage {
		this.mutex.Lock()
		call = this.pending[seq]
		delete(this.pending, seq)
	...
	case res.MessageStatusType() == rpc.Error:
