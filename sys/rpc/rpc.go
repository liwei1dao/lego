...
func newSys(options *Options) (sys *rpc, err error) {
	...
	if options.ConnectType == rpccore.Tcp {
		options.ServiceNode.Set(core.ServiceNode_Addr, options.CommAddrs[0])
	}
	...
	for i, v := range currnodes {
		nodes[i], _ = core.NewServiceNode(v.Value)
	}
	sys.selector, err = selector.NewSelector(nodes)
	return
}
...
func (this *rpc) Start() (err error) {
	...
	if err = this.options.Discovery.Start(this.ServiceNode()); err != nil {
		return
	}

	currnodes := this.options.Discovery.GetServices()
	if len(currnodes) > 0 {
		nodes := make([]core.IServiceNode, len(currnodes))
		for i, v := range currnodes {
			nodes[i], _ = core.NewServiceNode(v.Value)
		}
		this.selector.UpdateServer(nodes)
	...
	go func() { //监控服务发现
		var add, del, change []core.IServiceNode
		for v := range this.options.Discovery.WatchService() {
			nodes := make([]core.IServiceNode, len(v))
			for i, v := range v {
				nodes[i], _ = core.NewServiceNode(v.Value)
			}
			add, del, change = this.selector.UpdateServer(nodes)
			if len(add) > 0 { //发现节点
...
func (this *rpc) Close() (err error) {
	if err = this.options.Discovery.Stop(); err != nil {
		return
	}
