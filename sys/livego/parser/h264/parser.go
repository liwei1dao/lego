...
func (this *Parser) parseSpecificInfo(src []byte) error {
	if len(src) < 9 {
		return decDataNil
	}
	sps := []byte{}
	...
	this.specificInfo = append(this.specificInfo, sps...)
	this.specificInfo = append(this.specificInfo, pps...)

	return nil
}

func (this *Parser) isNaluHeader(src []byte) bool {
...
func (this *Parser) naluSize(src []byte) (int, error) {
	if len(src) < naluBytesLen {
...
func (this *Parser) Parse(b []byte, isSeq bool, w io.Writer) (err error) {
	switch isSeq {
	case true:
		err = this.parseSpecificInfo(b)
	case false:
		// is annexb
		if this.isNaluHeader(b) {
			_, err = w.Write(b)
		} else {
			err = this.getAnnexbH264(b, w)
		}
	}
	return
}
