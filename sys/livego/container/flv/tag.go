...
func (tag *Tag) IsKeyFrame() bool {
	return tag.mediat.frameType == packet.FRAME_KEY
}

func (tag *Tag) IsSeq() bool {
	return tag.mediat.frameType == packet.FRAME_KEY &&
		tag.mediat.avcPacketType == packet.AVC_SEQHDR
}
...
func (tag *Tag) parseVideoHeader(b []byte) (n int, err error) {
	...
	n++
	if tag.mediat.frameType == packet.FRAME_INTER || tag.mediat.frameType == packet.FRAME_KEY {
