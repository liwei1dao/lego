...
func (muxer *Muxer) Mux(p *packet.Packet, w io.Writer) error {
	first := true
	wBytes := 0
	pesIndex := 0
	tmpLen := byte(0)
	...
	pts := dts
	pid := audioPID
	var videoH packet.VideoPacketHeader
	if p.IsVideo {
		pid = videoPID
		videoH, _ = p.Header.(packet.VideoPacketHeader)
...
func (header *pesHeader) packet(p *packet.Packet, pts, dts int64) error {
	//PES header
	i := 0
	header.data[i] = 0x00
	i++
